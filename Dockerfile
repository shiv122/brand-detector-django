...
# image. psycopg2-binary (Postgres), PyMySQL + cryptography (MySQL/MariaDB —
# cryptography is required for MySQL 8's caching_sha2_password auth) and boto3
# (DigitalOcean Spaces) are runtime-only extras added on top of the locked set;
# the active DB driver is picked by DATABASE_URL.
#
...
RUN --mount=type=cache,target=/root/.cache/uv,sharing=locked \
 ...
 && uv pip install --python /app/.venv/bin/python psycopg2-binary==2.9.9 PyMySQL==1.1.1 cryptography==44.0.0 boto3==1.34.162 \
...
ENV DJANGO_SETTINGS_MODULE=detector.settings \
    ...
    DETECTION_WORKERS=1 \
    REAPER_INTERVAL=30 \
    DETECTION_HEARTBEAT_STALE_SECONDS=120 \
